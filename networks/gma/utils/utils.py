...
#         # augmented indices
#         aug_coord_img_1 = (coord_img_1[:, 0:1] * W1 +
#                            coord_img_1[:, 1:2]).long()
...
def downflow8(flow, mode='bilinear'):
    ...
    return F.interpolate(flow, size=new_size, mode=mode,
                         align_corners=True) / 8
...
def compute_inverse_interpolation_img(weights, indices, img, b, h_i, w_i):
    ...
    img_out.scatter_add_(1, ff_idx[(..., ) + (None, ) * k].expand_as(img_0),
                         img_0)
    img_out.scatter_add_(1, cf_idx[(..., ) + (None, ) * k].expand_as(img_1),
                         img_1)
    img_out.scatter_add_(1, fc_idx[(..., ) + (None, ) * k].expand_as(img_2),
                         img_2)
    img_out.scatter_add_(1, cc_idx[(..., ) + (None, ) * k].expand_as(img_3),
                         img_3)

    return img_out  # [b, h_i*w_i, ...]


def backwarp(image, flow, interp_mode='bilinear', padding_mode='zeros'):
    '''Backward warping.
    Inputs:
        image: (N,C,H,W)
        flow: (N,2,H,W)
